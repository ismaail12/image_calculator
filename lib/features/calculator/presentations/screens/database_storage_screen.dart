...
class DatabaseStorageScreen extends StatelessWidget {
  const DatabaseStorageScreen({super.key});

  @override
  Widget build(BuildContext context) {
    context.read<GetCalculationResultCubit>().fetchResults(StorageType.database);
          ...
          return RefreshIndicator(
            onRefresh: () async {
              context
                  ...
                  .fetchResults(StorageType.database);
            },
            child: ListView.builder(
              itemCount: state.fromDatabase.length,
              itemBuilder: (context, index) {
                final calculation = state.fromDatabase[index];
                return ListTile(
                  onTap: () {
